# Extended streaming GP algorithm of InfraredGP for streaming GP
...
if __name__ == '__main__':
    ...
    acc_emb = None
    brich_clus = Birch(n_clusters=None, threshold=0.6, branching_factor=50)
    for s in range(num_step):
        ...
        if acc_emb is None:
            ...
            acc_emb = np.concatenate((acc_emb, crt_emb[node_idx_s:node_idx_e, :]), axis=0)

        # ====================
        time_s = timeit.default_timer()
        brich_clus.partial_fit(crt_emb[node_idx_s:node_idx_e, :])
        crt_clus_res = brich_clus.predict(crt_emb)
        time_e = timeit.default_timer()
